...
export function LoginForm() {
  const { login, isOAuthAuthenticated, nextAuthSession, isLoading } = useSession();
  const router = useRouter();

  const handleLogin = async (role: 'driver' | 'admin') => {
    try {
      await login(role);
      // The router.push will be handled by the page component when user state changes
    } catch (error) {
  ...
  // Show Google OAuth login buttons
  return (
    <div className="space-y-6">
      <div className="text-center">
        <p className="text-gray-600 dark:text-gray-400 mb-6">
